...
impl Service {
	pub fn update_notification_counts(&self, user_id: &UserId, room_id: &RoomId) -> Result<()> {
		...
		let read_event =
			services().rooms.edus.read_receipt.private_read_get(room_id, user_id).unwrap_or(None).unwrap_or(0_u64);
		...
		services()
			.rooms
			.timeline
			.pdus_after(user_id, room_id, PduCount::min())? // what am i doing wrong
			//.find_map(|pdu| {
			...
			.filter(|pdu| {
				...
				user_id != pdu.sender
					&& services().rooms.short.get_or_create_shorteventid(&pdu.event_id).unwrap_or(0) != read_event
			})
			//.into_iter()
			.filter_map(|pdu| {
				...
				for action in services()
					.pusher
					.get_actions(user_id, &rules_for_user, &power_levels, &pdu.to_sync_room_event(), &pdu.room_id)
					.ok()?
				{
					match action {
		...
		self.db.update_notification_counts(user_id, room_id, notification_count, highlight_count)
	}
